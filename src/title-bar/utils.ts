...
export const getCurrentRef = (
  ...
  if (overflow) {
    return overflow.index === index ? overflowRef : childRefs[index > overflow.index ? index - 1 : index]
  }
...
export const getValidItem = (menu: MenuItem[], selected: number[], prev = false) => {
