...
import { Arrow, Unchecked, RadioUnchecked, RadioChecked, Checked } from './icons'
...
const getStatusIcon = (item: MenuItem) => {
  switch (item.type) {
    ...
    default: {
      if (item?.icon) {
        return <div className={styles.MenuItemIcon} style={{ backgroundImage: `url(${item.icon})` }} />
      }
      return <span />
    }
  }
...
const MenuItemComponent = ({ item, onClick, currentWindow, depth, dispatch, selectedPath, idx }: MenuItemProps) => {
  if (item.hidden === true) {
    return null
  }
  const theme = useContext(ThemeContext)
  if (isItemSeparator(item)) {
    return <hr className={styles.Separator} style={{ borderBottom: `1px solid ${theme.menu.separator!.color}` }} />
  }
  ...
  const selectedSub = selected && currentSelected(selectedPath, depth + 1) >= -1
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={styles.Accelerator} style={{ color: acceleratedColor }}>
          {item.accelerator}
        </span>
